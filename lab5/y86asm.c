...
symbol_t *find_symbol(char *name)
{
	...
	if (strncmp(sym->name, name, len) == 0)
	    return sym;
        sym=sym->next;
    }
    return NULL;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    for(i=0;i<strlen(cur);i++){
        if(!IS_LETTER(cur+i) && !IS_DIGIT(cur+i))
            break;
    }
    char *symbol = malloc(i+1);
    memset(symbol, '\0', i+1);
    strncpy(symbol,cur,i);
...
cont:
    ...
    switch (HIGH(inst->code)) {
      ...
      case I_RMMOVL: {  /* 4:0 regA, D(regB) - e.g., rmmovl %eax, 8(%esp)  */
        ...
        int i;
        for (i=0; i<4; i++)
            y86bin->codes[i+2]=(dest>>(8*i))&0xFF;
        goto cont;
      }
      ...
      case I_DIRECTIVE: {
        ...
        switch (LOW(inst->code)) {
          case D_DATA: {    /* .long data - e.g., .long 0xC0 */
            char *symbol;
            long value;
            ret=parse_data(&cur,&symbol,&value);
            if (ret == PARSE_ERR){
                line->type = TYPE_ERR;
          ...
          case D_POS: {   /* .pos D - e.g., .pos 0x100 */
            long addr;
            ret=parse_digit(&cur,&addr);
            if (ret == PARSE_ERR){
                line->type = TYPE_ERR;
                err_print("Invald pos");
                goto out;
            }
            vmaddr = addr;
            y86bin->addr = addr;
          ...
          case D_ALIGN: {   /* .align D - e.g., .align 4 */
            ...
            if (vmaddr%align != 0)
                vmaddr = vmaddr + align - vmaddr % align;
            y86bin->addr = vmaddr;
            goto cont;
          }
          default:
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        bin_t *y86bin=rtmp->y86bin;
        long *tmp;
        itype_t type=HIGH(y86bin->codes[0]);
        if (type == I_IRMOVL) 
            tmp=(long *)&y86bin->codes[2];
        else if (type == I_JMP || type == I_CALL)
            tmp=(long *)&y86bin->codes[1];
        else
            tmp=(long *)&y86bin->codes[0];
        *tmp=sym->addr;
        /* next */
        rtmp = rtmp->next;
    }
