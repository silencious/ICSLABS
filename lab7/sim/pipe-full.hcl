...
intsig D_stat   'if_id_curr->status'    # stat of D stage
...
int f_pc = [
    ...
    D_icode == IJXX && D_ifun == C_LE && !(E_icode in { IOPL, IIADDL }) && M_icode == IMRMOVL && !M_Cnd : D_valP;

	# Mispredicted branch.  Fetch at incremented PC
    # to exclude the situation above, check if there's bubble in pipe
	M_icode == IJXX && !M_Cnd && D_stat == SBUB : M_valA;

    # 'pushl+ret', forward valA
    D_icode == IRET && E_icode == IPUSHL : E_valA;

	# Completion of RET instruction.
	W_icode == IRET && M_stat == SBUB : W_valM;
...
bool D_bubble =
    ...
    !(E_ifun == C_LE && !(M_icode in {IOPL, IIADDL}) && W_icode == IMRMOVL) ||
